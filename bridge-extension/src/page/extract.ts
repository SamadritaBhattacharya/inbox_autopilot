/**
 * Stage 1 — pull candidate elements out of the live page.
 *
 * The walk itself runs **in the page**, injected through `Runtime.evaluate`. Everything the
 * funnel needs — box, computed style, role, accessible name, tree position — is available
 * there in one round trip, in a form that reads like what it is. The alternative,
 * re-stitching `DOMSnapshot.captureSnapshot`'s parallel arrays and joining them to the
 * accessibility tree by backend node id, is precise and a great deal of fragile plumbing.
 *
 * The trade worth stating: this code runs *in* the page, so a hostile page could lie to it.
 * That changes nothing about the safety model. The page was already the untrusted input; a
 * lying page can only alter what the agent *perceives*, and every irreversible consequence
 * of a perception is gated behind a human decision downstream.
 *
 ...
 * approximates it.
 *
 * Ported from `backend/app/surface/extract.py`. The injected source is kept byte-comparable
 * with the Python copy so the two surfaces perceive the same page the same way.
 */
import type { MailView, PageMeta, RawElement } from "../funnel/raw";
...
/** The walker, as source. Evaluated in the page; never imported into the worker. */
...
(maxNodes) => {
  ...
  function nameOf(el, interactiveHint) {
    ...
    // model a number it cannot reason about.
    if (interactiveHint) {
  ...
  // A secret must never leave the page, and this is the earliest point at which that can be
  // guaranteed. Everything downstream — the funnel, the vault, the wire, the model — only
  // ever sees what this function returns, so redacting HERE is the one place the guarantee
  // is total. The PII vault cannot help: it tokenizes so values can be resolved again later,
  ...
  for (const el of all) {
    ...
    // "is this reachable?" — geometric overlap only approximates it, and gets a full-viewport
    // overlay wrong in exactly the case that matters (an open dialog).
  ...
  //
  // A presence check is wrong here, and wrong in the direction that matters: the compose
  // markup sits in the DOM permanently, hidden by an ancestor, so it would report "compose
  // is open" on a plain inbox forever. A hidden element has a zero-size box — including when
  // an ancestor is what hides it — so measuring is the reliable test.
...
/** The shape `EXTRACT_JS` returns. Untrusted — it came from the page. */
...
/**
 * Coerce the page's output into `RawElement`s.
 *
 * Defensive on every field. This data came from a page we do not control, and a hostile or
 * merely broken one returning `{x: "banana"}` must produce a boring element rather than an
 * exception that kills the turn.
 */
export function parseElements(raw: unknown[]): RawElement[] {
  const out: RawElement[] = [];
  for (const entry of raw) {
    if (!entry || typeof entry !== "object") continue;
    const item = entry as Record<string, unknown>;
    const nodeId = num(item.nodeId, -1);
    if (nodeId < 0) continue;

    const parentId = item.parentId;
    const receives = item.receivesPointer;

    out.push({
      nodeId,
      role: str(item.role, "generic") || "generic",
      name: str(item.name),
      ...
      displayed: item.displayed === undefined ? true : Boolean(item.displayed),
      paintOrder: num(item.paintOrder),
      receivesPointer: typeof receives === "boolean" ? receives : null,
 ...
 *
 * Derived from the URL, which stays **extension-side**: a URL is an identifier, which is why
 * the observation carries an opaque `contextId` instead. The derived view is a small,
 * non-identifying fact the agent genuinely needs — "am I in a thread or a list?" — and it is
 * safe to send precisely because it is a category, not a reference.
 */
export function detectView(url: string, composeOpen: boolean): MailView {
  if (composeOpen) return "compose";
  const lowered = url.toLowerCase();

  // Signed out, or bounced to Google's login. This has to come first and be explicit: every
  // branch below eventually falls through to "inbox", so a sign-in wall was being reported
  // as a mailbox. The agent then dutifully "summarized the inbox" from a page that said
  // "Couldn't sign you in" — six steps spent on a confident, entirely fictional answer.
  if (
  ...
  const fragments: Array<[string, MailView]> = [
    ["#sent", "sent"],
    ["#drafts", "drafts"],
    ["#search", "search"],
    ["/calendar", "calendar"],
  ];
  for (const [fragment, view] of fragments) {
    if (lowered.includes(fragment)) return view;
  }
  ...
  if (!lowered.includes("#")) return "inbox";
  const fragment = lowered.slice(lowered.lastIndexOf("#") + 1);
  if (fragment.includes("/")) {
    const last = fragment.slice(fragment.lastIndexOf("/") + 1);
    if (last.length >= 8) return "thread";
  }
  return "inbox";
}
...
export function parseMeta(
  ...
  const url = str(raw.contextRef);
  const composeOpen = Boolean(raw.composeOpen);
  return {
    contextRef: url,
    title: str(raw.title),
    viewportWidth: Math.round(num(raw.viewportWidth, 1280)),
    viewportHeight: Math.round(num(raw.viewportHeight, 800)),
    scrollX: Math.round(num(raw.scrollX)),
    scrollY: Math.round(num(raw.scrollY)),
    view: detectView(url, composeOpen),
